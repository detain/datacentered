...
$db = $GLOBALS['tf']->db;
$db->query("select vps_id,vps_ip,vps_name,vps_root from vps_masters left join vps_master_details using (vps_id) where vps_type=11 limit 3");
while ($db->next_record(MYSQL_ASSOC)) {
	$service_master = $db->Record;
	...
	$factory->createClient($url)->then(
		function (Client $client) use ($service_master) {
			$result = $client->soapCall('GetVMList', ['hyperVAdmin' => 'Administrator', 'adminPassword' => $service_master['vps_root']]);
			echo "Got Result " . var_export($result) . PHP_EOL;
			/*
				function ($result) {
