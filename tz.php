...
$filename = ['autoload.php','Autoloader.php'];
...
function cpuinfo()
{
    ...
    if (in_array($os_real[$os_count], ['armv6l','armv7l','armv8l','mips','mipsel','aarch64'])) {
        ...
        switch ($os[0]) {
            ...
            default:
                ...
                foreach (array_count_values($model[1]) as $model_k => $model_v) {
                    $models[] = ['model' => $model_k, 'total' => $model_v, 'key' => array_search($model_k, $model[1])];
                }
	...
	<table>
        ...
        if (empty($disFuns)) {
            ...
            for ($i=0;$i<count($disFuns_array);$i++) {
                if ($i != 0 && $i % 5 == 0) {
                    array_splice($disFuns_array, $i, 0, ['<br/>']);
                }
            }
